locals {
  ...
  config_annotation = "${var.annotation_domain}/${var.extension_name}"
  env_vars = setunion(
    toset(var.env_vars),
    toset([{
      name  = "ANNOTATION"
...
resource "google_pubsub_subscription" "main" {
  for_each             = { for entry in var.cloud_deploy_extension_subscriptions : entry.topic => entry }
  project              = var.project_id
  name                 = "${var.extension_name}-${each.value.topic}-push"
  topic                = each.value.topic
  filter               = each.value.filter
  ack_deadline_seconds = 30
  labels = {
    repo_url       = "github0com_computeclub_gcp-cloud-deploy-extensions"
...
resource "google_cloud_run_v2_service" "main" {
  ...
  template {
    ...
    containers {
      image = "${local.artifact_registry_repo_endpoint}/${var.extension_name}:latest"
      dynamic "env" {
